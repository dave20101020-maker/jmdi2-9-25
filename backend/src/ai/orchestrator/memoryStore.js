 ...
 */
export function updateConversationHistory(memory, pillar, userMessage, assistantMessage) {
  ...
  if (memory.pillars[pillar].lastMessages.length > 20) {
    memory.pillars[pillar].lastMessages = memory.pillars[pillar].lastMessages.slice(-20);
  }
...
export function markTopicCovered(memory, pillar, topic) {
  ...
  if (!memory.pillars[pillar].coveredTopics.some(t => t.topic === topic)) {
    ...
    if (memory.pillars[pillar].coveredTopics.length > 50) {
      memory.pillars[pillar].coveredTopics = memory.pillars[pillar].coveredTopics.slice(-50);
    }
  }
