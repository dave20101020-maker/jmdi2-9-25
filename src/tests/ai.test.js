...
import GuidedJournal from '../src/ai/GuidedJournal';
import AIInsights from '../src/ai/AIInsights';
...
describe('GuidedJournal Component', () => {
  ...
  test('displays error when save attempted with empty response', async () => {
    ...
    await waitFor(() => {
      expect(toast.error).toHaveBeenCalledWith('Please write something before saving');
    });
  });
  ...
  test('has proper ARIA labels for accessibility', () => {
    ...
    expect(screen.getByLabelText(/How do you feel after writing/i)).toBeInTheDocument();
  });
});
...
describe('AIInsights Component', () => {
  ...
  test('renders loading state initially', () => {
    ...
    const loadingElements = screen.queryAllByText(/loading|generating|analyzing/i);
    expect(loadingElements.length).toBeGreaterThan(0);
  });

  test('renders error state when API fails', async () => {
    const { api } = require('../src/utils/apiClient');
    api.generateInsights = jest.fn().mockRejectedValue(new Error('API Error'));
  ...
  test('displays success state with insights', async () => {
    const { api } = require('../src/utils/apiClient');
    api.generateInsights = jest.fn().mockResolvedValue({
    ...
    await waitFor(() => {
      expect(screen.getByText(/You are doing well on sleep/i)).toBeInTheDocument();
    });
  });
  ...
  test('displays recommendations when available', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText(/Exercise 30 minutes daily/i)).toBeInTheDocument();
    });
  });

  test('handles no data gracefully', async () => {
    const { api } = require('../src/utils/apiClient');
    api.generateInsights = jest.fn().mockResolvedValue(null);
