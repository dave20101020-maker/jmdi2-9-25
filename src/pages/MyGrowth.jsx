...
export default function MyGrowth() {
  ...
  const { data: plans = [], isLoading: plansLoading } = useQuery({
    ...
    queryFn: () =>
      api.getPlans({ created_by: user?.email }, "-created_date", 100),
    enabled: !!user,
    initialData: [],
  });
    ...
    onSuccess: (data, variables) => {
      ...
      if (variables.data.linkedPlanId || data.linkedPlanId) {
        ...
        queryClient.invalidateQueries({
          queryKey: [
            "lifePlan",
            variables.data.linkedPlanId || data.linkedPlanId,
          ],
        });
      }
    },
  ...
  const deleteGoalMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      if (data.linkedPlanId) {
        queryClient.invalidateQueries({ queryKey: ["lifePlans", user?.email] });
        queryClient.invalidateQueries({
          queryKey: ["lifePlan", data.linkedPlanId],
        });
      }
    },
  });
  ...
  const handleToggleHabitComplete = (habit) => {
    ...
    if (isCompletedToday) {
      ...
      toast.success(
        `Habit completed — momentum: ${newStreakCount} day${
          newStreakCount !== 1 ? "s" : ""
        }`
      );
    }
  ...
  Object.keys(PILLARS).forEach((pillarId) => {
    ...
    const pillarHabits = habits.filter(
      (h) => h.pillar === pillarId && h.isActive
    );
              ...
              <div className="text-center py-16">
                <div className="bg-white/5 border border-white/10 rounded-2xl p-12 max-w-md mx-auto">
                  ...
                  <p className="text-white/70 mb-6">
                    Create goals, plans, and habits to track your growth across
                    all life pillars
                  </p>
                      ...
                      <div className="flex items-center gap-3 mb-4">
                        ...
                        <div>
                          ...
                          <p className="text-white/60 text-sm">
                            {data.goals.length} goals • {data.plans.length}{" "}
                            plans • {data.habits.length} habits
                          </p>
                        </div>
                      </div>
                          ...
                          <div className="space-y-2">
                            {data.plans.map((plan) => (
                              <Link
                                key={plan.id}
                                to={
                                  createPageUrl("PlanDetail") + `?id=${plan.id}`
                                }
            ...
            {goals.length === 0 ? (
              <div className="text-center py-16">
                <div className="bg-white/5 border border-white/10 rounded-2xl p-12 max-w-md mx-auto">
                  ...
                  <p className="text-white/70 mb-6">
                    Create your first SMART goal and let our AI help you achieve
                    it!
                  </p>
                  <Button
                    onClick={() => setShowGoalCreator(true)}
            ...
            {plans.length === 0 ? (
              ...
              <div className="space-y-3">
                {plans.map((plan) => {
                  ...
                  const planGoals = goals.filter(
                    (g) => g.linkedPlanId === plan.id
                  );
              ...
              <div className="grid md:grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <p className="text-white/70">
                    Specific, measurable targets with clear success criteria and
                    deadlines. Perfect for concrete achievements.
                  </p>
                </div>
                <div>
                  <h4 className="text-white/90 font-semibold mb-1 flex items-center gap-2">
                  ...
                  <p className="text-white/70">
                    Comprehensive strategies with daily habits, weekly actions,
                    and long-term vision. Great for lifestyle changes.
                  </p>
                </div>
              </div>
              <p className="text-white/60 text-sm mt-3">
                💡 Tip: Link goals to plans to create a powerful growth
                framework!
              </p>
            </div>
          </div>
