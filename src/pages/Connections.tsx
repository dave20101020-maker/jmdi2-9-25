...
export default function Connections() {
  ...
  const sentRequests = allConnections.filter(
    (c) => c.status === "pending" && c.userId === user?.email
  );
  const receivedRequests = allConnections.filter(
  ...
  const handleBlockUser = (connectionId) => {
    if (
      confirm("Block this user? They will not be able to send you requests.")
    ) {
      updateConnectionMutation.mutate({
        id: connectionId,
        data: { status: "blocked" },
                      ...
                      <div className="bg-gradient-to-br from-blue-500/10 to-purple-600/10 border border-blue-500/30 rounded-xl p-4 md:p-6 text-left">
                        ...
                        <ul className="space-y-2 text-xs md:text-sm text-white/80">
                          ...
                          <li className="flex items-start gap-2">
                            ...
                            <span>
                              View your friends' progress on the leaderboard
                            </span>
                          </li>
                          <li className="flex items-start gap-2">
                            <span className="text-[#FFD700] flex-shrink-0 mt-0.5">
              ...
              <div className="space-y-3">
                {filteredFriends.map((conn) => (
                  <FriendCard
                    ...
                    onUpdate={(data) =>
                      updateConnectionMutation.mutate({ id: conn.id, data })
                    }
                  />
                ))}
              </div>
        ...
        {activeTab === "find" && (
          <div className="space-y-6">
            <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl p-4 md:p-6">
              ...
              <div className="space-y-4">
                <div>
                  <Label className="text-white mb-2 block text-sm md:text-base">
                    Friend's Email
                  </Label>
                  <Input
                    type="email"
                ...
                <Button
                  ...
                  disabled={
                    !searchEmail.trim() || createConnectionMutation.isPending
                  }
            ...
            <div className="bg-blue-500/10 border border-blue-500/30 rounded-2xl p-4 md:p-6">
              ...
              <ul className="space-y-2 text-sm text-white/80">
                <li className="flex items-start gap-2">
                  ...
                  <span>
                    Enter your friend's email address and send a request
                  </span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-blue-400 mt-1 flex-shrink-0">2.</span>
                  <span>
                    They'll receive the request and can accept or decline
                  </span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-blue-400 mt-1 flex-shrink-0">3.</span>
                  <span>
                    Once accepted, you can view each other's public milestones
                  </span>
                </li>
              </ul>
            </div>
                        ...
                        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                          <div className="flex items-center gap-3 flex-1 min-w-0">
                            ...
                            <span className="text-white font-medium break-all">
                              {safeText(blockedEmail)}
                            </span>
                          </div>
                          <Button
                            onClick={() =>
                              deleteConnectionMutation.mutate(blocked.id)
                            }
                            size="sm"
                            variant="ghost"
...
function FriendCard({ connection, currentUserEmail, onRemove, onUpdate }) {
  ...
  const friendEmail =
    connection.userId === currentUserEmail
      ? connection.friendEmail
      : connection.userId;
  const friendName =
    connection.userId === currentUserEmail ? connection.friendName : "";
  const displayName = connection.nickname || friendName || friendEmail;
  const sanitizedDisplayName = safeText(displayName);
  const sanitizedEmail = safeText(friendEmail);
          ...
          {isEditing ? (
            ...
            <>
              <div className="flex flex-col sm:flex-row sm:items-center gap-2 mb-1">
                <h4 className="text-white font-bold text-base md:text-lg break-all">
              ...
              <div className="text-white/40 text-xs">
                Connected since{" "}
                {format(new Date(connection.created_date), "MMM d, yyyy")}
              </div>
            </>
          )}
        </div>
