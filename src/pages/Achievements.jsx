...
export default function Achievements() {
  ...
  const { data: connections = [] } = useQuery({
    ...
    queryFn: async () => {
      const sent = await api.getConnections({
        userId: user.email,
        status: "accepted",
      });
      const received = await api.getConnections({
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="flex gap-2 overflow-x-auto pb-2 mb-6">
            {Array.from({ length: 6 }).map((_, idx) => (
              <Skeleton
                key={idx}
                className="h-9 w-24 flex-shrink-0 rounded-full"
              />
            ))}
          </div>
  ...
  const daysSinceJoined = user.created_date
    ? differenceInDays(new Date(), new Date(user.created_date))
    : 0;
  ...
  const userStats = {
    ...
    maxPillarScore,
    allPillarsAbove70:
      todayEntries.length === 8 && todayEntries.every((e) => e.score >= 70),
    lifeScore:
