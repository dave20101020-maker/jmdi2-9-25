...
import { LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, RadarChart, PolarGrid, PolarAngleAxis, PolarRadiusAxis, Radar, ScatterChart, Scatter, ZAxis } from "recharts";
import { format, subDays, differenceInDays, parseISO } from "date-fns";
import { TrendingUp, TrendingDown, ArrowRight, Plus, GripVertical, X, Maximize2, Flame, Trophy, Smile, Lightbulb, Calendar } from "lucide-react";
...
const PILLARS = [
  ...
  { id: "physical_health", name: "Physical Health", color: "#FF7F50", icon: "❤️" },
...
function TrendChart({ entries, selectedPillar, timeRange, customStartDate, customEndDate, accessiblePillars }) {
  let chartData = [];
  let days;
  ...
  return (
    <div className="relative">
      {Math.abs(weekChange) > 0.1 && chartData.length >= 14 && (
        <div className={`absolute top-2 right-2 z-10 flex items-center gap-1 px-3 py-1 rounded-full text-xs font-bold ${
          weekChange > 0 
    ...
    >
      <div className="flex items-center justify-between mb-3">
        ...
        <div className={`px-2 py-1 rounded-full text-xs font-bold ${
          correlation.strength > 0.7 ? 'bg-green-500/20 text-green-400' :
          correlation.strength > 0.5 ? 'bg-yellow-500/20 text-yellow-400' :
...
function ScatterPlotModal({ correlation, onClose, entries }) {
  const scatterData = entries
    .filter(e => e.pillar === correlation.pillar1Id || e.pillar === correlation.pillar2Id)
    .reduce((acc, entry) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-[#0A1628]/95 backdrop-blur-sm flex items-center justify-center p-6" role="dialog" aria-modal="true" aria-labelledby="scatter-title">
      <div className="bg-[#1a1f35] border border-white/20 rounded-2xl p-6 max-w-2xl w-full">
        <div className="flex items-start justify-between mb-6">
          <div>
            <h2 id="scatter-title" className="text-2xl font-bold text-white mb-2">
          ...
          <button onClick={onClose} aria-label="Close scatter plot" className="text-white/60 hover:text-white">
        ...
        <ResponsiveContainer width="100%" height={400}>
          <ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
            ...
            <XAxis 
              ...
              label={{ value: correlation.pillar1, position: 'bottom', fill: 'rgba(255,255,255,0.7)' }}
            />
            <YAxis 
              type="number" 
              dataKey="y" 
              ...
              stroke="rgba(255,255,255,0.5)"
              tick={{ fill: 'rgba(255,255,255,0.7)' }}
              label={{ value: correlation.pillar2, angle: -90, position: 'left', fill: 'rgba(255,255,255,0.7)' }}
            />
            <ZAxis range={[100, 100]} />
            <Tooltip
        ...
        <Button onClick={onClose} className="w-full mt-4 bg-white/10 hover:bg-white/20 text-white">
          Close
        </Button>
      </div>
    </div>
...
function WidgetStreak({ user }) {
  return (
    <div className="bg-gradient-to-br from-orange-500/20 to-red-500/20 border border-orange-500/40 rounded-2xl p-6">
      ...
      <div className="text-5xl font-bold text-orange-400">{user?.streak_days || 0}</div>
...
function WidgetPoints({ user }) {
  return (
    <div className="bg-gradient-to-br from-[#D4AF37]/20 to-[#F4D03F]/20 border border-[#D4AF37]/40 rounded-2xl p-6">
      ...
      <div className="text-5xl font-bold text-[#F4D03F]">{user?.points || 0}</div>
...
function WidgetMood({ recentMoods }) {
  const avgMood = recentMoods.length > 0
    ? Math.round(recentMoods.reduce((sum, m) => sum + m.moodScore, 0) / recentMoods.length)
    : 50;
  
  const moodEmoji = avgMood > 75 ? '😄' : avgMood > 50 ? '🙂' : avgMood > 25 ? '😐' : '😟';
...
function WidgetRadar({ entries, accessiblePillars }) {
  ...
  const radarData = accessiblePillars.map(pillar => {
    const entry = entries.find(e => e.pillar === pillar.id && e.date === today);
    return {
    ...
    <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
      ...
      <ResponsiveContainer width="100%" height={250}>
        <RadarChart data={radarData}>
          ...
          <PolarRadiusAxis domain={[0, 100]} tick={{ fill: 'rgba(255,255,255,0.5)' }} />
          <Radar
            name="Score"
            dataKey="score"
            stroke="#D4AF37"
...
export default function Analytics() {
  ...
  useEffect(() => {
    async function getUser() {
      try {
        ...
        setUser(currentUser);
        setWidgets(currentUser.dashboard_widgets || ["streak", "points", "mood", "radar"]);
      } catch (error) {
  ...
  const generateCorrelations = async () => {
    ...
    try {
      ...
      for (let i = 0; i < accessiblePillars.length; i++) {
        for (let j = i + 1; j < accessiblePillars.length; j++) {
          ...
          const pairs = dates.map(date => {
            const e1 = entries.find(e => e.date === date && e.pillar === p1.id);
            const e2 = entries.find(e => e.date === date && e.pillar === p2.id);
      ...
      for (const pair of topPairs) {
        try {
          const prompt = `Generate a brief, actionable insight (max 100 chars) about the correlation between ${pair.pillar1} and ${pair.pillar2} with strength ${pair.strength.toFixed(2)}. Focus on what the user can do.`;
          
          const result = await api.aiCoach({ prompt });
          pair.insight = result.output || result || `${pair.pillar1} and ${pair.pillar2} are strongly connected`;
        } catch (error) {
  ...
  const saveWidgetsMutation = useMutation({
    mutationFn: (newWidgets) => api.authUpdateMe({ dashboard_widgets: newWidgets }),
    onSuccess: () => {
  ...
  const renderWidget = (widgetId) => {
    switch (widgetId) {
      ...
      case "radar":
        return <WidgetRadar entries={entries} accessiblePillars={accessiblePillars} />;
      default:
        return null;
    }
  };
  ...
  return (
    <div className="min-h-screen pb-24 px-6 pt-8">
      <div className="max-w-6xl mx-auto">
        ...
        <header className="mb-8">
          <div className="flex items-center gap-2 mb-2">
            <h1 className="text-3xl md:text-4xl font-bold text-white">Insights</h1>
            <HelpTooltip
          ...
          <p className="text-white/70">Discover patterns in your growth journey</p>
        </header>
        
        {/* Section 1: Trend Carousel */}
        <section className="mb-8" aria-labelledby="trends-heading">
          <div className="flex items-center gap-2 mb-4">
            <h2 id="trends-heading" className="text-xl font-bold text-white">Trends</h2>
            <HelpTooltip
              title="Trend Charts"
          ...
          {/* Time Range Chips */}
          <div className="flex gap-2 mb-4 overflow-x-auto pb-2" role="tablist" aria-label="Time range selection">
          ...
          {timeRange === "custom" && (
            <div className="bg-white/5 border border-white/10 rounded-xl p-4 mb-4 animate-in slide-in-from-top duration-200">
              <div className="flex flex-col sm:flex-row items-start sm:items-end gap-4">
                <div className="flex-1 w-full">
                  <Label className="text-white mb-2 block text-sm font-bold flex items-center gap-2">
                    Start Date
                    <HelpTooltip
                ...
                <div className="flex-1 w-full">
                  <Label className="text-white mb-2 block text-sm font-bold">End Date</Label>
                  <Input
                    type="date"
                    value={customEndDate}
                ...
                <Button
                  ...
                  className="bg-gradient-to-r from-[#D4AF37] to-[#F4D03F] text-[#0A1628] font-bold whitespace-nowrap"
                >
                  Apply Range
                </Button>
              </div>
              ...
              {customStartDate && customEndDate && (
                <div className="mt-3 text-sm text-white/60 flex items-center gap-2">
                  ...
                  Showing {differenceInDays(parseISO(customEndDate), parseISO(customStartDate)) + 1} days
                  ({format(parseISO(customStartDate), 'MMM d')} - {format(parseISO(customEndDate), 'MMM d, yyyy')})
                </div>
              )}
            </div>
          )}
          ...
          {/* Pillar Chips */}
          <div className="flex gap-2 mb-6 overflow-x-auto pb-2" role="tablist" aria-label="Pillar selection">
            {PILLARS.map(pillar => {
              if (pillar.id !== 'overall' && !accessiblePillars.find(p => p.id === pillar.id)) {
                return null;
              }
              return (
                <button
                  ...
                  style={
                    selectedPillar === pillar.id
                      ? { backgroundColor: `${pillar.color}20`, borderColor: pillar.color, color: pillar.color }
                      : { color: '#fff' }
                  }
                >
        ...
        {correlations.length > 0 && (
          <section className="mb-8" aria-labelledby="discoveries-heading">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                <h2 id="discoveries-heading" className="text-xl font-bold text-white">AI Discoveries</h2>
                <HelpTooltip
            ...
            <div className="flex gap-4 overflow-x-auto pb-4" role="list" aria-label="Correlation insights">
        ...
        <section className="mb-8" aria-labelledby="dashboard-heading">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              <h2 id="dashboard-heading" className="text-xl font-bold text-white">Your Dashboard</h2>
              <HelpTooltip
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-[#D4AF37] to-[#F4D03F] text-[#0A1628] rounded-full font-bold hover:shadow-lg transition-all"
            >
                    ...
                    <Draggable key={widgetId} draggableId={widgetId} index={index}>
                      {(provided) => (
                        ...
                        >
                          ...
                          >
                            <GripVertical className="w-5 h-5 text-white/60" aria-hidden="true" />
                          </div>
                          <button
        ...
        <section className="bg-blue-500/10 border border-blue-500/30 rounded-2xl p-6">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h3 className="text-lg font-bold text-white mb-3">Understanding Your Analytics</h3>
              <div className="space-y-3 text-sm text-white/80">
                <div>
                  <strong className="text-white">Trends:</strong> Track how each pillar changes over time. Look for upward trends as signs of progress!
                </div>
                <div>
                  <strong className="text-white">Custom Date Ranges:</strong> Select specific periods to analyze (e.g., a vacation, stressful work week, or training period). Max 365 days.
                </div>
                <div>
                  <strong className="text-white">Correlations:</strong> When two pillars move together, improving one often helps the other. Use your strongest areas to boost weaker ones.
                </div>
                <div>
                  <strong className="text-white">Week-over-Week (WoW):</strong> Shows percentage change from last week. Green = improvement, Red = needs attention.
                </div>
                <div>
                  <strong className="text-white">Radar Chart:</strong> Shows balance across all pillars. A balanced shape means well-rounded growth!
                </div>
              </div>
            </div>
          </div>
      ...
      {/* Add Widget Modal */}
      {showAddWidget && (
        <div className="fixed inset-0 z-50 bg-[#0A1628]/95 backdrop-blur-sm flex items-center justify-center p-6" role="dialog" aria-modal="true" aria-labelledby="add-widget-title">
          <div className="bg-[#1a1f35] border border-white/20 rounded-2xl p-6 max-w-md w-full">
            <div className="flex items-start justify-between mb-6">
              <h2 id="add-widget-title" className="text-2xl font-bold text-white">Add Widget</h2>
              <button onClick={() => setShowAddWidget(false)} aria-label="Close add widget dialog" className="text-white/60 hover:text-white">
                <X className="w-6 h-6" aria-hidden="true" />
              </button>
            </div>
            
            <div className="grid grid-cols-2 gap-3" role="list" aria-label="Available widgets">
              {AVAILABLE_WIDGETS.map(widget => (
                ...
                >
                  <div className="text-3xl mb-2" aria-hidden="true">{widget.icon}</div>
                  <div className="text-white font-bold text-sm">{widget.label}</div>
                  {widgets.includes(widget.id) && (
      ...
      {loadingCorrelations && (
        <AIThinkingOverlay isVisible={true} message="Analyzing patterns in your data..." />
      )}
    </div>
  );
}
