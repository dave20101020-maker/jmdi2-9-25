...
import { Brain, BookOpen, Wind, Lightbulb, Heart, TrendingUp, Sparkles, Plus, Quote } from "lucide-react"; // Added Quote
...
import { LineChart, Line, XAxis, YAxis, Tooltip, ResponsiveContainer } from 'recharts'; // Added Recharts imports
...
export default function MentalHealth() {
  ...
  const saveJournalMutation = useMutation({
    mutationFn: (data) => api.createJournal({
      userId: user.email,
      date: today,
      ...data
    }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['journalEntries', user?.email] });
  ...
  const saveThoughtMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['thoughtRecords', user?.email] });
  ...
  const hoursAgoJournal = latestJournal 
    ? Math.round(differenceInHours(new Date(), new Date(latestJournal.updated_date || latestJournal.created_date)))
    : null;

  const moodTrend = moodEntries.length >= 2 
    ? (moodEntries[0].rating || 0) > (moodEntries[1].rating || 0) ? "up" : 
      (moodEntries[0].rating || 0) < (moodEntries[1].rating || 0) ? "down" : "stable"
    : "stable";

  const thoughtRecordConsistency = thoughtRecords.length >= 7 
    ? Math.round((thoughtRecords.filter(t => {
  ...
  const recentActivityData = [
    ...journals.slice(0, 2).map(j => ({
      ...
      icon: BookOpen,
      title: "Journal Entry",
      summary: `"${j.response?.substring(0, 60)}${j.response?.length > 60 ? '...' : ''}" • Mood: ${j.mood}/10`,
      timestamp: j.date,
      color: "#4CC9F0",
      badges: [
        { text: `Mood ${j.mood}/10`, color: "#4CC9F0" },
        ...(j.tags || []).slice(0, 2).map(tag => ({ text: tag, color: "#7C3AED" }))
      ],
      data: j
    })),
    ...thoughtRecords.slice(0, 2).map(t => ({
      ...
      summary: `Situation: "${t.situation?.substring(0, 50)}${t.situation?.length > 50 ? '...' : ''}"`,
    ...
    ...meditationLogs.slice(0, 1).map(m => ({
      ...
      summary: `${m.duration} min${m.completedFully ? ' • Completed' : ' • Partial'}`,
      timestamp: m.created_date,
      color: "#7C3AED",
      badges: [
        { text: `${m.duration} min`, color: "#7C3AED" },
        { text: m.completedFully ? 'Complete' : 'Partial', color: m.completedFully ? "#52B788" : "#FFD700" }
      ],
      data: m
    }))
  ...
  return (
    <PillarPage pillar={PILLAR} title="Mental Health" subtitle="Journal, reflect, and reframe thoughts" stats={stats}>
      ...
      {weeklyMoodData.some(d => d.mood !== null) && (
        ...
        >
          ...
          <div className="text-center text-white/60 text-sm mt-2">
            {weeklyMoodData.filter(d => d.mood !== null).length} of 7 days tracked
          </div>
        </div>
      )}
      ...
      {lastBalancedThought && (
        <div className="bg-gradient-to-br from-white/5 to-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-6 mb-6 relative overflow-hidden"
        ...
        >
          ...
          <div className="flex items-start gap-3">
            <div className="w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0"
              style={{ backgroundColor: '#FFD70020', border: '1px solid #FFD70040' }}
            >
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-2">
                <h3 className="text-white font-bold">Latest Reframed Thought</h3>
              ...
              <div className="bg-white/5 border border-white/10 rounded-lg p-3 mb-2">
                ...
                <p className="text-white/90 text-sm italic">"{lastBalancedThought.automaticThought}"</p>
              </div>
              ...
              <div className="bg-gradient-to-br from-green-500/10 to-emerald-500/10 border border-green-500/30 rounded-lg p-3">
                ...
                <p className="text-white text-sm font-medium">"{lastBalancedThought.balancedThought}"</p>
              </div>
            </div>
          </div>
        </div>
      ...
      {activeView === "overview" && (
        <div className="space-y-4 md:space-y-6">
          ...
          {journals.length > 0 && (
            <DataCard title="Recent Journal Entries" titleIcon={<BookOpen />} color="#4CC9F0">
                  ...
                  <div key={entry.id} className="bg-white/5 border border-white/10 rounded-xl p-4">
                    <div className="flex items-start justify-between mb-2">
                      <div className="flex-1">
                        ...
                        <p className="text-white/60 text-sm italic line-clamp-2">"{entry.prompt}"</p>
                      </div>
                      {entry.mood && (
                        <div className="text-blue-400 font-bold">😊 {entry.mood}/10</div>
                      )}
                    </div>
                    <p className="text-white/80 text-sm line-clamp-2 mt-2">{entry.response}</p>
                    {entry.tags?.length > 0 && (
                      <div className="flex gap-1 mt-2">
                        {entry.tags.map(tag => (
                          <span key={tag} className="px-2 py-0.5 bg-blue-500/20 text-blue-400 text-xs rounded-full">
                            {tag}
                          </span>
                        ))}
                      </div>
          ...
          {thoughtRecords.length > 0 && (
            <DataCard title="Recent Thought Records" titleIcon={<Brain />} color="#7C3AED">
              <div className="space-y-3">
                {thoughtRecords.slice(0, 3).map(thought => {
                  const emotionsBefore = thought.emotions || [];
                  const emotionsAfter = thought.emotionsAfter || [];
                  const avgBefore = emotionsBefore.length > 0
                    ? emotionsBefore.reduce((sum, e) => sum + e.intensity, 0) / emotionsBefore.length
                    : 0;
                  const avgAfter = emotionsAfter.length > 0
                    ? emotionsAfter.reduce((sum, e) => sum + e.intensity, 0) / emotionsAfter.length
                    : avgBefore;
                  const improved = avgAfter < avgBefore;

                  return (
                    <div key={thought.id} className="bg-white/5 border border-white/10 rounded-xl p-4">
                      <div className="flex items-start justify-between mb-2">
                        <div className="flex-1">
                          <div className="text-white/70 text-sm mb-1">
                            {format(new Date(thought.timestamp), 'MMM d - h:mm a')}
                          </div>
                          <p className="text-white/80 text-sm line-clamp-2 italic">"{thought.automaticThought}"</p>
                        </div>
                        {improved && (
      ...
      {activeView === "journal" && (
        <div>
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-6">
            <h2 className="text-lg md:text-xl font-bold text-white">Journal Entries</h2>
            <Button
              onClick={() => setShowJournal(true)}
          ...
          {journals.length === 0 ? (
            <div className="text-center py-12 md:py-16 bg-white/5 border border-white/10 rounded-2xl px-4">
              ...
              <h3 className="text-xl md:text-2xl font-bold text-white mb-2">Start Journaling</h3>
              <p className="text-white/70 text-sm mb-6 max-w-md mx-auto">
                Get daily AI-generated prompts to explore your thoughts and emotions
              </p>
              <Button
                onClick={() => setShowJournal(true)}
            ...
            <div className="space-y-3 md:space-y-4">
              {journals.map(entry => (
                <div key={entry.id} className="bg-[#1a1f35] border border-white/20 rounded-2xl p-4 md:p-5 backdrop-blur-sm">
                    ...
                    <div className="flex-1">
                      ...
                      {entry.tags?.length > 0 && (
                        <div className="flex flex-wrap gap-1 mt-2">
                          {entry.tags.map(tag => (
                            <span key={tag} className="px-2 py-1 bg-blue-500/20 text-blue-400 text-xs rounded-full capitalize">
                              {tag}
                            </span>
                          ))}
                        </div>
                  ...
                  <p className="text-white/80 text-sm md:text-base leading-relaxed">{entry.response}</p>
                </div>
              ))}
            </div>
          )}
      ...
      {activeView === "thoughts" && (
        <div>
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-6">
            <h2 className="text-lg md:text-xl font-bold text-white">Thought Records (CBT)</h2>
            <Button
              onClick={() => setShowThoughtRecorder(true)}
          ...
          {thoughtRecords.length === 0 ? (
            <div className="text-center py-12 md:py-16 bg-white/5 border border-white/10 rounded-2xl px-4">
              ...
              <h3 className="text-xl md:text-2xl font-bold text-white mb-2">Challenge Negative Thoughts</h3>
              <p className="text-white/70 text-sm mb-6 max-w-md mx-auto">
                Use CBT techniques to identify, examine, and reframe unhelpful thinking patterns
              </p>
              <Button
                onClick={() => setShowThoughtRecorder(true)}
            ...
            <div className="space-y-3 md:space-y-4">
              {thoughtRecords.map(thought => {
                ...
                const emotionsAfter = thought.emotionsAfter || [];
                const avgBefore = emotionsBefore.length > 0
                  ? Math.round(emotionsBefore.reduce((sum, e) => sum + e.intensity, 0) / emotionsBefore.length)
                  : 0;
                const avgAfter = emotionsAfter.length > 0
                  ? Math.round(emotionsAfter.reduce((sum, e) => sum + e.intensity, 0) / emotionsAfter.length)
                  : avgBefore;
                const improved = avgAfter < avgBefore;

                return (
                  <div key={thought.id} className="bg-[#1a1f35] border border-white/20 rounded-2xl p-4 md:p-5 backdrop-blur-sm">
                    <div className="flex flex-col md:flex-row md:items-start justify-between gap-3 mb-4">
                      <div className="flex-1">
                        ...
                        <div className="text-white/70 text-sm">
                          Situation: <span className="text-white">{thought.situation}</span>
                        </div>
                      </div>
                      {improved && (
                        <div className="px-3 py-1 bg-green-500/20 border border-green-500/40 rounded-full flex items-center gap-1 self-start">
                          <TrendingUp className="w-4 h-4" />
                          <span className="text-green-400 text-sm font-bold">Improved</span>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="space-y-3">
                      <div className="bg-red-500/10 border border-red-500/30 rounded-lg p-3">
                        <div className="text-red-400 text-xs mb-1">Automatic Thought:</div>
                        <p className="text-white/90 text-sm italic">"{thought.automaticThought}"</p>
                      </div>

                      {thought.balancedThought && (
                        <div className="bg-green-500/10 border border-green-500/30 rounded-lg p-3">
                          <div className="text-green-400 text-xs mb-1">Balanced Thought:</div>
                          <p className="text-white/90 text-sm">"{thought.balancedThought}"</p>
                        </div>
                      )}
                      ...
                      {emotionsBefore.length > 0 && (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                          <div>
                            <div className="text-white/60 text-xs mb-2">Emotions Before:</div>
                          ...
                          {emotionsAfter.length > 0 && (
                            <div>
                              <div className="text-white/60 text-xs mb-2">After Reframing:</div>
                              {emotionsAfter.map((em, idx) => (
                                <div key={idx} className="text-green-400 capitalize">
                                  {em.emotion}: {em.intensity}/10
                                </div>
                              ))}
                            </div>
      ...
      {activeView === "tools" && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
          <div className="bg-[#1a1f35] border border-white/20 rounded-2xl p-6 backdrop-blur-sm"
            style={{ boxShadow: '0 0 25px rgba(82, 183, 136, 0.15)' }}
          >
            <Wind className="w-12 h-12 text-green-400 mb-4" style={{ filter: 'drop-shadow(0 0 8px rgba(82, 183, 136, 0.6))' }} />
            <h3 className="text-lg md:text-xl font-bold text-white mb-2">Breathing Exercises</h3>
          ...
          >
            <Brain className="w-12 h-12 text-purple-400 mb-4" style={{ filter: 'drop-shadow(0 0 8px rgba(124, 58, 237, 0.6))' }} />
            <h3 className="text-lg md:text-xl font-bold text-white mb-2">CBT Thought Records</h3>
            <p className="text-white/70 text-sm mb-6">
              Challenge and reframe negative thinking patterns with proven CBT techniques
            </p>
            <ul className="space-y-2 text-sm text-white/80 mb-6">
              <li className="flex items-start gap-2">
      ...
      {selectedLog && detailModalType === 'journal' && (
        <LogDetailModal
          ...
          title="Journal Entry"
          fields={[
            { key: 'prompt', label: 'Prompt', icon: Sparkles, color: "#7C3AED" },
            { key: 'response', label: 'Your Reflection', icon: BookOpen, color: "#4CC9F0" },
            { key: 'mood', label: 'Mood After', icon: Heart, color: "#FF69B4", unit: '/10' }
          ]}
        />
      )}
      ...
      {selectedLog && detailModalType === 'thought' && (
        <LogDetailModal
          ...
          fields={[
            ...
            { key: 'situation', label: 'Situation', icon: Brain, color: "#FF5733" },
            { key: 'automaticThought', label: 'Automatic Thought', icon: Brain, color: "#FFD700" },
            { 
              key: 'emotions', 
              label: 'Emotions', 
              icon: Heart, 
              color: "#FF69B4", 
              render: (emotions) => emotions && emotions.length > 0 ? (
                <div className="space-y-1">
                  {emotions.map((e, i) => (
                    <div key={i} className="flex items-center justify-between bg-white/5 rounded-lg p-2">
                      <span className="text-white/90 text-sm">{e.emotion}</span>
                      <span className="text-pink-400 font-bold">{e.intensity}/10</span>
                    </div>
                  ))}
                </div>
              ) : null 
            },
            { key: 'evidence', label: 'Evidence For', icon: TrendingUp, color: "#FF5733" },
            { key: 'counterEvidence', label: 'Evidence Against', icon: TrendingUp, color: "#52B788" },
            { key: 'balancedThought', label: 'Balanced Thought', icon: Lightbulb, color: "#4CC9F0" },
            { key: 'aiSuggestion', label: 'AI Suggestion', icon: Sparkles, color: "#7C3AED" }
          ]}
        />
      )}
    </PillarPage>
