...
import { createPageUrl, PILLARS, handleHabitCompletion } from "@/utils";
import {
  ArrowLeft,
  CheckCircle2,
  Circle,
        ...
        <div className="flex-1">
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className="text-white font-medium">
              ...
              {format(new Date(review.weekEndDate), "MMM d")}
            </span>
            {review.overallRating && (
              <div className="flex items-center gap-1">
                <span className="text-xl">
                  {getRatingEmoji(review.overallRating)}
                </span>
...
export default function PlanDetail() {
  ...
  const { data: habits = [], isLoading: habitsLoading } = useQuery({
    ...
    queryFn: () =>
      api.getHabits(
        { linkedPlanId: planId, userId: user?.email },
        "-created_date",
        100
      ),
    enabled: !!planId && !!user,
  ...
  const { data: weeklyReviews = [], isLoading: reviewsLoading } = useQuery({
    ...
    queryFn: () =>
      api.getWeeklyReviews(
        { planId: planId, created_by: user?.email },
        "-weekEndDate",
        20
      ),
    enabled: !!planId && !!user,
  ...
  const deleteMutation = useMutation({
    mutationFn: async () => {
      ...
      const deleteReviewPromises = weeklyReviews.map((r) =>
  ...
  const handleDelete = () => {
    if (
      confirm(
        "Are you sure you want to delete this plan? This will also delete all associated habits and weekly reviews. Linked goals will remain but will be unlinked from this plan."
      )
    ) {
  ...
  const daysSinceStart = plan.startDate
    ? Math.floor(
        (new Date() - new Date(plan.startDate)) / (1000 * 60 * 60 * 24)
      )
    : 0;
  ...
  const habitContribution =
    totalHabits > 0
      ? `${todayCompleted}/${totalHabits} habits completed → +${
          todayCompleted * 5
        }/${totalHabits * 5} pts`
      : "No habits yet";
  ...
  const avgGoalProgress =
    linkedGoals.length > 0
      ? Math.round(
          linkedGoals.reduce((sum, g) => sum + (g.progress || 0), 0) /
            linkedGoals.length
        )
      : 0;
            ...
            <div className="mb-4 flex items-center gap-4">
              ...
              <div className="flex-1">
                ...
                <div className="text-xs text-white/60 mb-1">
                  {avgGoalProgress}% from {linkedGoals.length} linked goal
                  {linkedGoals.length !== 1 ? "s" : ""}
                </div>
                <div className="text-xs text-white/60">{habitContribution}</div>
              </div>
            </div>
          )}
          ...
          <div className="flex flex-wrap items-center gap-4 text-sm text-white/60">
            {plan.startDate && (
              <div className="flex items-center gap-1">
                ...
                {daysSinceStart > 0 && (
                  <span className="ml-1">({daysSinceStart} days ago)</span>
                )}
              </div>
            )}
            {planStreak > 0 && (
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-white/70 text-sm mb-3">
                  Create SMART goals linked to this plan for more focused
                  tracking
                </p>
                <Button
                  onClick={() => setShowGoalCreator(true)}
          ...
          <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl p-6 mb-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="text-sm text-white/60">
                ...
                {weeklyReviews.length === 1 ? "review" : "reviews"}
              </div>
            </div>
        ...
        {habits.length > 0 && (
          <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl p-6 mb-6">
            ...
            <p className="text-xs text-white/40">
              Tap to mark habits complete today • Each completion adds +5 points
              to this plan
            </p>
          </div>
        )}
