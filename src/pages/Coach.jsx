...
import { ArrowLeft, Send, Loader2, Target, CheckCircle2, Calendar, TrendingUp, TrendingDown, MessageSquare } from "lucide-react";
...
const PILLARS = {
  sleep: {
    ...
    quickQuestions: ["Why am I tired?", "Improve sleep quality", "Create a sleep plan"]
  },
  diet: {
    name: "Diet",
    color: "#52B788",
    ...
    quickQuestions: ["Healthy meal ideas", "Nutrition tips", "Create a nutrition plan"]
  },
  exercise: {
    name: "Exercise",
    color: "#FF5733",
  ...
  physical_health: {
    ...
    quickQuestions: ["Health checkup tips", "Preventive care", "Create a wellness plan"]
  },
  mental_health: {
    name: "Mental Health",
    color: "#4CC9F0",
    ...
    quickQuestions: ["Reduce stress", "Improve mood", "Create a mental wellness plan"]
  },
  finances: {
    name: "Finances",
    color: "#2E8B57",
    ...
    quickQuestions: ["Budget help", "Savings strategies", "Create a financial plan"]
  },
  social: {
    name: "Social",
    color: "#FFD700",
    ...
    quickQuestions: ["Improve relationships", "Social confidence", "Create a social plan"]
  },
  spirituality: {
    name: "Spirituality",
    color: "#7C3AED",
    ...
    quickQuestions: ["Find purpose", "Meditation guide", "Create a spiritual plan"]
  }
};

function SmartGoalDisplay({ goal, pillar }) {
  return (
    <div className="my-4 bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl p-6"
    ...
    >
      <div className="flex items-start gap-3 mb-4">
        ...
        <div className="flex-1">
          <h3 className="text-xl font-bold text-white mb-1">SMART Goal Created! 🎯</h3>
      ...
      <div className="space-y-3 ml-6">
        <div>
          <span className="text-green-400 font-semibold text-sm">Specific:</span>
          <p className="text-white/80 text-sm mt-1">{goal.specific}</p>
        </div>
        
        <div>
          <span className="text-blue-400 font-semibold text-sm">Measurable:</span>
          <p className="text-white/80 text-sm mt-1">{goal.measurable}</p>
        </div>
        
        {goal.achievable && (
          <div>
            <span className="text-purple-400 font-semibold text-sm">Achievable:</span>
        ...
        {goal.relevant && (
          <div>
            <span className="text-yellow-400 font-semibold text-sm">Relevant:</span>
            <p className="text-white/80 text-sm mt-1">{goal.relevant}</p>
          </div>
        )}
        
        <div>
          <span className="text-orange-400 font-semibold text-sm">Time-bound:</span>
  ...
  return (
    ...
    >
      <div className="flex items-start gap-3 mb-4">
        ...
        <div className="flex-1">
          <h3 className="text-xl font-bold text-white mb-1">{plan.planTitle}</h3>
...
export default function Coach() {
  ...
  const [aiThinkingMessage, setAiThinkingMessage] = useState('NorthStar is thinking...');
  ...
  const { data: previousInteractions = [] } = useQuery({
    ...
    queryFn: () => api.getCoachInteractions({ userId: user?.email, pillar: pillarId }, '-timestamp', 20),
    enabled: !!user,
    initialData: []
  });
  ...
  const messageLimit = isPremium ? 50 : 5;
  ...
  const buildUserContext = () => {
    ...
    const weekAgo = pillarEntries.find(e => e.date === format(subDays(new Date(), 7), 'yyyy-MM-dd'));
    
    // Calculate trend
    const currentScore = todayEntry?.score || (last7Days.length > 0 ? Math.round(last7Days.reduce((sum, e) => sum + e.score, 0) / last7Days.length) : 0);
    const weekAgoScore = weekAgo?.score || 0;
    const trend = weekAgoScore > 0 ? currentScore - weekAgoScore : 0;
    const trendLabel = trend > 5 ? 'improving' : trend < -5 ? 'declining' : 'steady';
    ...
    return {
      ...
      trend,
      trendLabel,
      last7DaysAvg: last7Days.length > 0 ? Math.round(last7Days.reduce((sum, e) => sum + e.score, 0) / last7Days.length) : 0,
  ...
  const detectPlanRequest = (text) => {
    const planKeywords = ['create a plan', 'make a plan', 'plan for', 'help me plan', 'build a plan'];
    return planKeywords.some(kw => text.toLowerCase().includes(kw));
  };
  
  const detectGoalRequest = (text) => {
    const goalKeywords = ['set a goal', 'create a goal', 'my goal', 'want to achieve', 'help me'];
  ...
  const sendMessage = async (text) => {
    ...
    if (dailyMessageCount >= messageLimit) {
      toast.error(`Daily limit reached (${messageLimit} messages)`, {
        description: isPremium ? 'Try again tomorrow' : 'Upgrade to Premium for 50 messages/day'
      });
      return;
    }
    ...
    try {
      ...
      const contextString = `
USER CONTEXT:
- Current ${pillar.name} score: ${context.currentScore}/100 (${context.trendLabel} trend, ${trend > 0 ? '+' : ''}${context.trend} vs last week)
- Last 7 days average: ${context.last7DaysAvg}/100
- Recent entries: ${context.recentEntries.length > 0 ? context.recentEntries.map(e => `${e.date}: ${e.score}/100${e.notes ? ' - ' + e.notes : ''}`).join('; ') : 'No recent data'}
- Active goals: ${context.goals.length > 0 ? context.goals.map(g => g.goalStatement).join('; ') : 'None'}
...
- Motivation: ${context.userCapability.motivation?.motivationLevel || 'unknown'}/5

PREVIOUS ADVICE GIVEN (Do NOT repeat):
${context.recentAdvice.length > 0 ? context.recentAdvice.map((adv, i) => `${i + 1}. ${adv}`).join('\n') : 'None yet'}

CONVERSATION HISTORY:
${previousInteractions.slice(0, 3).map(i => `User: ${i.userMessage}\nYou: ${i.aiResponse.substring(0, 200)}...`).join('\n\n')}`;
...
Respond with ONLY valid JSON (no markdown, no text before/after):
{
  ...
  "successMetrics": ["2-3 ways to measure progress"]
        ...
        await api.createCoachInteraction({
          ...
          adviceGiven: [...(planData.dailyActions || []), ...(planData.goals || [])],
          contextData: context,
          timestamp: new Date().toISOString()
        });
        
        setMessages(prev => [...prev, { role: 'assistant', content: planData, isPlan: true }]);
...
Respond with ONLY valid JSON:
{
  ...
  "timeBound": "Specific timeframe (e.g., '30 days', 'by Dec 15', '2 weeks')"
        ...
        setMessages(prev => [...prev, { role: 'assistant', content: goalData, isGoal: true }]);
        setDailyMessageCount(prev => prev + 1);
...
Keep it conversational and supportive.`;
        ...
        const advicePoints = aiResponse.match(/\d+\.\s+([^\n]+)/g)?.map(line => line.replace(/^\d+\.\s+/, '').substring(0, 100)) || [];
      ...
      <div className="flex flex-col h-screen">
        ...
        >
          ...
          <div className="text-right">
            ...
            <div className={`text-sm font-bold ${messagesRemaining <= 1 ? 'text-red-400' : 'text-[#D4AF37]'}`}>
            ...
            {messages.map((message, i) => (
              <div key={i}>
                {message.role === 'user' ? (
                  ...
                  <div className="flex justify-start">
                    <div className="max-w-[95%]">
                      <SmartGoalDisplay goal={message.content} pillar={pillar} />
                    </div>
                  </div>
                ) : (
                  <div className="flex justify-start">
            ...
            {messagesRemaining <= 1 && (
              <div className="mb-3 p-3 bg-orange-500/10 border border-orange-500/30 rounded-lg text-center">
                <p className="text-orange-400 text-sm font-medium">
                  {messagesRemaining === 0 ? '⚠️ Daily limit reached' : '⚠️ Last message for today'}
                  {!isPremium && ' • Upgrade to Premium for 50 messages/day'}
                </p>
              </div>
            )}
