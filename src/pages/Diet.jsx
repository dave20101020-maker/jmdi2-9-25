...
import { Utensils, Plus, Droplets, TrendingUp, Award, BarChart, Calendar, CheckCircle2, Target, Flame, Apple } from "lucide-react";
import { Button } from "@/components/ui/button";
import { format, differenceInHours, differenceInDays, startOfMonth } from "date-fns";
...
export default function Diet() {
  ...
  const saveMealMutation = useMutation({
    mutationFn: (data) => api.createMeal({
      userId: user.email,
      date: today,
      ...data
    }),
  ...
  const healthyMeals = recentMeals.filter(m => m.quality === 'healthy').length;
  const mealQualityRate = recentMeals.length > 0 
    ? Math.round((healthyMeals / recentMeals.length) * 100)
    : 0;

  const last7Days = waterLogs.slice(0, 7);
  const avgWater = last7Days.length > 0
    ? Math.round(last7Days.reduce((sum, w) => sum + (w.glassesConsumed || 0), 0) / last7Days.length * 10) / 10
    : 0;
  ...
  const hoursAgoMeal = latestMeal 
    ? Math.round(differenceInHours(new Date(), new Date(latestMeal.updated_date || latestMeal.created_date)))
    : null;

  const qualityTrend = meals.length >= 2
    ? meals.slice(0, 7).filter(m => m.quality === 'healthy').length >
      meals.slice(7, 14).filter(m => m.quality === 'healthy').length ? "up" :
      meals.slice(0, 7).filter(m => m.quality === 'healthy').length <
      meals.slice(7, 14).filter(m => m.quality === 'healthy').length ? "down" : "stable"
    : "stable";
  ...
  const monthlyHealthyRate = thisMonthMeals.length > 0
    ? Math.round((thisMonthMeals.filter(m => m.quality === 'healthy').length / thisMonthMeals.length) * 100)
    : 0;
  ...
  const atAGlanceMetrics = [
    ...
    },
    {
      ...
      message: todayWaterGlasses >= todayWaterGoal 
  ...
  const recentActivityData = [
    ...meals.slice(0, 4).map(m => {
      ...
      const qualityColor = m.quality === 'healthy' ? "#52B788" : m.quality === 'moderate' ? "#FFD700" : "#FF5733";
      
      return {
        id: m.id,
        type: 'meal',
        icon: Utensils,
        title: `${mealType?.emoji || '🍽️'} ${m.mealType.charAt(0).toUpperCase() + m.mealType.slice(1)}`,
        summary: `${m.quality} • ${m.description?.substring(0, 50)}${m.description?.length > 50 ? '...' : ''}`,
        timestamp: m.date,
        color: qualityColor,
        badges: [
          { text: m.quality, color: qualityColor },
          ...(m.satisfactionLevel ? [{ text: `${m.satisfactionLevel}/10 satisfaction`, color: "#FF69B4" }] : [])
        ],
        data: m
      };
    }),
    ...waterLogs.slice(0, 1).filter(w => w.glassesConsumed >= w.dailyGoal).map(w => ({
      id: w.id,
      type: 'water',
      icon: Droplets,
  ...
  return (
    <PillarPage pillar={PILLAR} title="Diet & Nutrition" subtitle="Track meals, water, and nutrition" stats={stats}>
      <PillarAtAGlance metrics={atAGlanceMetrics} color={PILLAR.color} />

      <MilestonesSection milestones={milestones} color={PILLAR.color} title="Nutrition Achievements" compact={true} />

      <PillarTip tips={getTipsForPillar('diet')} color={PILLAR.color} icon={Apple} title="Nutrition Tip" />
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-2 mt-4">
              {mealTypeData.map(meal => (
                <div key={meal.name} className="flex items-center gap-2 text-sm">
                ...
                return (
                  <div key={dateStr}>
                    <div className="flex items-center justify-between mb-1 text-xs">
                      <span className="text-white/70">{format(date, 'EEE, MMM d')}</span>
                      <span className="text-white/90 font-bold">{consumed}/{goal}</span>
                    </div>
                    <div className="w-full h-2 bg-white/10 rounded-full overflow-hidden">
                      <div 
                        ...
                        style={{ 
                          ...
                          backgroundColor: percentage >= 100 ? "#52B788" : percentage >= 70 ? "#4CC9F0" : "#FFD700"
                        }}
                      />
                    </div>
                  </div>
      ...
      {showMealLogger && (
        <MealLogger onClose={() => setShowMealLogger(false)} onSave={(data) => saveMealMutation.mutateAsync(data)} />
      )}

      {selectedLog && detailModalType === 'meal' && (
        <LogDetailModal log={selectedLog} onClose={() => { setSelectedLog(null); setDetailModalType(null); }} color={PILLAR.color} icon={Utensils} title="Meal Details" fields={[
          { key: 'mealType', label: 'Meal Type', icon: Calendar, color: "#FFD700", render: (value) => {
            const type = MEAL_TYPES.find(t => t.value === value);
            return <span className="text-lg capitalize text-white/90">{type?.emoji} {value}</span>;
          }},
          { key: 'quality', label: 'Quality', icon: TrendingUp, color: selectedLog.quality === 'healthy' ? "#52B788" : selectedLog.quality === 'moderate' ? "#FFD700" : "#FF5733", render: (value) => <span className="text-lg capitalize text-white/90">{value}</span> },
          { key: 'description', label: 'Description', icon: Utensils, color: "#4CC9F0" },
          { key: 'portionSize', label: 'Portion Size', icon: BarChart, color: "#7C3AED" },
          { key: 'satisfactionLevel', label: 'Satisfaction', icon: CheckCircle2, color: "#FF69B4", unit: '/10' },
          { key: 'ingredients', label: 'Key Ingredients', icon: Apple, color: "#52B788" }
        ]} />
      )}
    </PillarPage>
  );
