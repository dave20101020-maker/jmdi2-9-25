...
import { Moon, Plus, BookOpen, Clock, BedDouble, CheckCircle2, Flame, TrendingUp, Sparkles, Target, Sun, Thermometer, Volume2, Wind, Award, Zap, AlertCircle } from "lucide-react";
import { Button } from "@/components/ui/button";
import { format, subDays, startOfWeek, differenceInHours, addDays, startOfMonth, differenceInDays } from "date-fns";
...
const AUDIT_FACTORS = [
  ...
  { key: "temperature", label: "Temperature", icon: Thermometer, color: "#FF5733" },
  { key: "noise", label: "Noise", icon: Volume2, color: "#4CC9F0" },
  { key: "mattressComfort", label: "Comfort", icon: BedDouble, color: "#52B788" },
...
export default function Sleep() {
  ...
  const { data: routines = [] } = useQuery({
    ...
    queryFn: () => api.getBedtimeRoutines({ userId: user?.email }, '-created_date', 20),
    enabled: !!user,
    initialData: []
  });
  ...
  const { data: audits = [] } = useQuery({
    ...
    queryFn: () => api.getSleepEnvironmentAudits({ userId: user?.email }, '-date', 20),
    enabled: !!user,
    initialData: []
  });

  const saveJournalMutation = useMutation({
    mutationFn: (data) => api.createSleepJournal({
      userId: user.email,
      date: format(new Date(), 'yyyy-MM-dd'),
      ...data
    }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['sleepJournals', user?.email] });
  ...
  const saveRoutineMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['bedtimeRoutines', user?.email] });
  ...
  const saveAuditMutation = useMutation({
    mutationFn: (data) => api.createSleepEnvironmentAudit({
      userId: user.email,
      date: format(new Date(), 'yyyy-MM-dd'),
      ...data
    }),
  ...
  const handleToggleRoutineAdherence = (routine) => {
    ...
    if (isFollowedToday) {
      ...
      toast.success(`Routine followed! 🌙 ${newStreak} day streak`);
    }
  ...
  const avgSleepHours = recentEntries.length > 0
    ? Math.round(recentEntries.reduce((sum, e) => sum + (e.sleep_hours || 0), 0) / recentEntries.length * 10) / 10
    : 0;
  const avgQuality = recentEntries.length > 0
    ? Math.round(recentEntries.reduce((sum, e) => sum + (e.sleep_quality || 0), 0) / recentEntries.length * 10) / 10
    : 0;
  ...
  const hoursAgo = latestEntry
    ? Math.round(differenceInHours(new Date(), new Date(latestEntry.updated_date || latestEntry.created_date)))
    : null;

  const qualityTrend = sleepEntries.length >= 2
    ? (sleepEntries[0].sleep_quality || 0) > (sleepEntries[1].sleep_quality || 0) ? "up" :
      (sleepEntries[0].sleep_quality || 0) < (sleepEntries[1].sleep_quality || 0) ? "down" : "stable"
    : "stable";

  const monthStart = format(startOfMonth(new Date()), 'yyyy-MM-dd');
  const thisMonthEntries = sleepEntries.filter(e => e.date >= monthStart);
  const bestMonthScore = thisMonthEntries.length > 0
    ? Math.max(...thisMonthEntries.map(e => e.score || 0))
    : 0;

  const perfectNights = thisMonthEntries.filter(e =>
    (e.sleep_hours >= 7 && e.sleep_hours <= 9) && e.sleep_quality >= 8
  ).length;

  const bestRoutineStreak = activeRoutines.length > 0
  ...
  const milestones = [
    ...
    recentAuditScore >= 85 && {
      ...
      isNew: differenceInHours(new Date(), new Date(latestAudit?.created_date || new Date())) <= 48
    }
  ].filter(Boolean);
  ...
  const atAGlanceMetrics = [
    ...
    },
    {
      ...
      progress: activeRoutines.length > 0 ? (routinesFollowedToday / activeRoutines.length) * 100 : 0,
      message: routinesFollowedToday === activeRoutines.length && activeRoutines.length > 0
  ...
  const recentActivityData = [
    ...journals.slice(0, 2).map(j => ({
      ...
      icon: BookOpen,
      title: "Sleep Journal",
      summary: j.dreams ? `Dreamed: "${j.dreams.substring(0, 60)}${j.dreams.length > 60 ? '...' : ''}"` : `Quality: ${j.sleepQuality}/10`,
      timestamp: j.date,
      color: "#9370DB",
      badges: [
        { text: `Quality ${j.sleepQuality}/10`, color: "#9370DB" },
        ...(j.wokeUpFeeling ? [{ text: j.wokeUpFeeling, color: "#4CC9F0" }] : [])
      ],
      data: j
    })),
    ...
    ...audits.slice(0, 1).map(a => ({
      ...
      color: "#52B788",
      badges: [
        { text: `Score ${a.overallScore}`, color: a.overallScore >= 80 ? "#52B788" : a.overallScore >= 60 ? "#FFD700" : "#FF5733" }
      ],
      data: a
    }))
  ...
  return (
    <PillarPage pillar={PILLAR} title="Sleep Hub" subtitle="Track, journal, and optimize your sleep" stats={stats}>
      <AIContentButtons pillar="sleep" pillarName="Sleep" color={PILLAR.color} />
      
      <PillarAtAGlance metrics={atAGlanceMetrics} color={PILLAR.color} />

      <MilestonesSection milestones={milestones} color={PILLAR.color} title="Sleep Achievements" compact={true} />

      <PillarTip tips={getTipsForPillar('sleep')} color={PILLAR.color} icon={Sparkles} title="Sleep Pro Tip" />
        ...
        >
            ...
            <AreaChart data={sleepTrendData}>
              <defs>
                <linearGradient id="sleepGradient" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="5%" stopColor={PILLAR.color} stopOpacity={0.8}/>
                  <stop offset="95%" stopColor={PILLAR.color} stopOpacity={0.1}/>
                </linearGradient>
              </defs>
              <XAxis 
                dataKey="date" 
          ...
          <div className="grid grid-cols-2 gap-2 mt-4 text-xs">
            ...
            <div className="bg-white/5 rounded-lg p-2 text-center">
              ...
              <div className="text-white font-bold">{sleepTrendData.filter(d => d.hours !== null).length}/14</div>
            </div>
          </div>
        </div>
      )}
      ...
      >
        ...
        <div className="flex flex-col md:flex-row items-center justify-around gap-8">
          ...
          {activeRoutines.length > 0 && (
            <CircularProgress
              value={activeRoutines.reduce((sum, r) => sum + (r.currentStreak || 0), 0)}
      ...
      <div className="grid md:grid-cols-3 gap-4 mb-6">
        ...
        <ActionCard
          ...
          stats={latestAudit ? `Score: ${latestAudit.overallScore}/100` : 'Not audited'}
          color="#52B788"
      ...
      {activeRoutines.length > 0 && (
        <DataCard title="Your Bedtime Routines" titleIcon={<Clock />} color="#4CC9F0">
          <div className="space-y-3">
            {activeRoutines.slice(0, 2).map(routine => {
              const isFollowedToday = routine.adherenceDates?.includes(today);
              
              return (
                <div key={routine.id} className="bg-white/5 border border-white/10 rounded-xl p-4">
                  <div className="flex items-center justify-between mb-3">
                    <div>
                      <h4 className="text-white font-bold">{routine.routineName}</h4>
                  ...
                  <div className="flex items-center gap-2 text-xs">
                    {routine.steps?.slice(0, 3).map((step, idx) => (
                      <span key={idx} className="px-2 py-1 bg-purple-500/20 text-purple-400 rounded-lg">
                        {step.action}
                      </span>
                    ))}
                    {routine.steps?.length > 3 && (
                      <span className="text-white/40">+{routine.steps.length - 3} more</span>
                    )}
                  </div>
                </div>
              );
      ...
      {/* Modals */}
      {showJournalModal && (
        <SleepJournalEntry onClose={() => setShowJournalModal(false)} onSave={(data) => saveJournalMutation.mutateAsync(data)} />
      )}

      {showRoutineModal && (
        <RoutineBuilder onClose={() => { setShowRoutineModal(false); setEditingRoutine(null); }} onSave={(data) => saveRoutineMutation.mutateAsync(data)} initialRoutine={editingRoutine} />
      )}

      {showAuditModal && (
        <EnvironmentAudit onClose={() => setShowAuditModal(false)} onSave={(data) => saveAuditMutation.mutateAsync(data)} />
      )}

      {selectedLog && detailModalType === 'journal' && (
        <LogDetailModal log={selectedLog} onClose={() => { setSelectedLog(null); setDetailModalType(null); }} color="#9370DB" icon={BookOpen} title="Sleep Journal Entry" fields={[
          { key: 'sleepQuality', label: 'Sleep Quality', icon: Sparkles, color: "#9370DB", unit: '/10' },
          { key: 'wokeUpFeeling', label: 'Woke Up Feeling', icon: Sun, color: "#FFD700", render: (value) => <span className="text-lg capitalize text-white/90">{value}</span> },
          { key: 'dreams', label: 'Dreams', icon: Moon, color: "#6B46C1" },
          { key: 'disturbances', label: 'Disturbances', icon: Volume2, color: "#FF5733" }
        ]} />
      )}

      {selectedLog && detailModalType === 'audit' && (
        <LogDetailModal log={selectedLog} onClose={() => { setSelectedLog(null); setDetailModalType(null); }} color="#52B788" icon={BedDouble} title="Sleep Environment Audit" fields={[
          { key: 'overallScore', label: 'Overall Score', icon: Target, color: selectedLog.overallScore >= 80 ? "#52B788" : selectedLog.overallScore >= 60 ? "#FFD700" : "#FF5733", unit: '/100' },
          ...AUDIT_FACTORS.map(factor => ({ key: factor.key, label: factor.label, icon: factor.icon, color: factor.color, unit: '/10' })),
          { key: 'issuesIdentified', label: 'Issues Identified', icon: AlertCircle, color: "#FF5733" },
          { key: 'improvements', label: 'Suggested Improvements', icon: Sparkles, color: "#4CC9F0" }
        ]} />
      )}

      {selectedLog && detailModalType === 'entry' && (
        <LogDetailModal log={selectedLog} onClose={() => { setSelectedLog(null); setDetailModalType(null); }} color={PILLAR.color} icon={Moon} title="Sleep Log" fields={[
          { key: 'sleep_hours', label: 'Hours Slept', icon: Moon, color: PILLAR.color, unit: 'h' },
          { key: 'sleep_quality', label: 'Sleep Quality', icon: Sparkles, color: "#7C3AED", unit: '/10' },
          { key: 'score', label: 'Overall Score', icon: Target, color: "#D4AF37", unit: '/100' }
        ]} />
      )}
    </PillarPage>
  );
