...
import { CheckCircle2, Circle, Flame, Award, Target, TrendingUp } from "lucide-react";
...
export default function DailyProgress() {
  ...
  const { data: habits = [], isLoading: habitsLoading } = useQuery({
    queryKey: ['habits'],
    queryFn: () => api.getHabits({ userId: user?.email, isActive: true }, '-created_date', 100),
    enabled: !!user,
    initialData: []
  });
  ...
  const { data: plans = [], isLoading: plansLoading } = useQuery({
    ...
    queryFn: () => api.getPlans({ created_by: user?.email, isActive: true }, '-created_date', 100),
    enabled: !!user,
    initialData: []
  });
  ...
  const activeHabits = habits.filter(h => activePlanIds.includes(h.linkedPlanId));
    ...
    return (
      <div className="min-h-screen pb-24 px-6 pt-8">
        <div className="max-w-2xl mx-auto">
          <h1 className="text-3xl md:text-4xl font-bold text-white mb-8 text-center">Daily Progress</h1>
          <div className="text-center py-12">
            <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl p-8">
              <Target className="w-16 h-16 mx-auto mb-4 text-[#D4AF37]" />
              <h2 className="text-xl font-bold text-white mb-2">No Active Habits Yet</h2>
              <p className="text-white/60 mb-6">
                Create a plan with your AI coach to get personalized daily habits!
              </p>
            </div>
          </div>
        </div>
    ...
    <div className="min-h-screen pb-24 px-6 pt-8">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="text-center mb-8">
          <h1 className="text-3xl md:text-4xl font-bold text-white mb-2">Daily Progress</h1>
          <p className="text-white/60">{format(new Date(), 'EEEE, MMMM d, yyyy')}</p>
        </div>
        
        {/* Overall Progress */}
        <div className="bg-gradient-to-br from-[#D4AF37]/20 to-[#F4D03F]/20 backdrop-blur-md border border-[#D4AF37]/30 rounded-2xl p-6 mb-8"
        ...
        >
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="text-center">
              ...
              <div className="text-xs text-white/60">{todayProgress}/{totalHabits}</div>
            </div>
          </div>
          ...
          {progressPercent === 100 && (
            <div className="mt-4 text-center">
              <div className="inline-flex items-center gap-2 px-4 py-2 bg-green-500/20 border border-green-500/40 rounded-full">
                ...
                <span className="text-green-400 font-bold">All habits completed! 🎉</span>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="space-y-6">
          {Object.entries(habitsByPlan).map(([planId, planHabits]) => {
            ...
            const planCompleted = planHabits.filter(h => isCompletedToday(h)).length;
                  ...
                  {planHabits.map(habit => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex-1">
                          <div className={`font-medium mb-1 ${completed ? 'text-white/80 line-through' : 'text-white'}`}>
                            {habit.habitText}
                          </div>
                          ...
                          <div className="flex items-center gap-3 text-xs">
                            ...
                            {streak >= 21 && (
                              <div className="flex items-center gap-1 text-purple-400">
                                ...
                                <span className="font-semibold">21-Day Master!</span>
                              </div>
                            )}
                            {streak >= 7 && streak < 21 && (
                              <div className="flex items-center gap-1 text-blue-400">
                                <Award className="w-3 h-3" />
                                <span className="font-semibold">Week Warrior!</span>
                              </div>
                            )}
                            {streak >= 3 && streak < 7 && (
                              <div className="flex items-center gap-1 text-green-400">
                                <Award className="w-3 h-3" />
                                <span className="font-semibold">3-Day Strong!</span>
                              </div>
                            )}
                            
                            {!completed && streak === 0 && (
                              <span className="text-white/40">Start your streak today!</span>
                            )}
                          </div>
                        </div>
                      </button>
        ...
        <div className="mt-8 text-center bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl p-6">
          ...
          <p className="text-white/60 text-sm">
            Small daily actions lead to big long-term results. Keep going! 🌟
          </p>
        </div>
      </div>
    </div>
