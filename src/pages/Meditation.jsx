...
import { Wind, Plus, Sparkles, TrendingUp, Calendar, Heart, Target, CheckCircle2, Award, Flame, Clock, Brain } from "lucide-react";
...
export default function Meditation() {
  ...
  const { data: sessions = [] } = useQuery({
    ...
    queryFn: () => api.getMeditationLogs({ userId: user?.email }, '-timestamp', 100),
    enabled: !!user,
    initialData: []
  });

  const saveSessionMutation = useMutation({
    mutationFn: (data) => api.logMeditation({
      userId: user.email,
      date: today,
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['meditationSessions', user?.email] });
      setShowLogger(false);
  ...
  const avgCalmImprovement = recentSessions.length > 0
    ? Math.round(recentSessions.reduce((sum, s) => sum + ((s.calmAfter || 0) - (s.calmBefore || 0)), 0) / recentSessions.length * 10) / 10
    : 0;
  ...
  const avgFocusQuality = recentSessions.length > 0
    ? Math.round(recentSessions.reduce((sum, s) => sum + (s.focusQuality || 0), 0) / recentSessions.length * 10) / 10
    : 0;

  const latestSession = sessions.length > 0 ? sessions[0] : null;
  const hoursAgo = latestSession 
    ? Math.round(differenceInHours(new Date(), new Date(latestSession.timestamp)))
    : null;
  ...
  const calmTrendData = Array.from({ length: 14 }, (_, i) => {
    ...
    const avgImprovement = daySessions.length > 0
      ? daySessions.reduce((sum, s) => sum + ((s.calmAfter || 0) - (s.calmBefore || 0)), 0) / daySessions.length
      : null;
  ...
  const thisMonthMinutes = thisMonthSessions.reduce((sum, s) => sum + (s.duration || 0), 0);
  ...
  const atAGlanceMetrics = [
    {
      ...
      trend: meditationStreak >= 7 ? "up" : meditationStreak >= 3 ? "stable" : "down",
      lastUpdated: hoursAgo ? `${hoursAgo}h ago` : "No sessions",
      message: meditationStreak === 0 ? "🧘 Start your meditation journey today" : null
    },
    {
      icon: <Sparkles />,
      ...
      trend: avgCalmImprovement >= 2 ? "up" : avgCalmImprovement >= 1 ? "stable" : "down"
    },
    {
      icon: <Target />,
      label: "Focus Quality",
  ...
  const recentActivityData = sessions.slice(0, 5).map(s => {
    ...
    return {
      ...
      type: 'session',
      icon: Wind,
      title: s.guidedSessionName || `${s.technique.replace('_', ' ')} Meditation`,
      summary: `${s.duration} min • Calm: ${s.calmBefore}→${s.calmAfter} (+${calmChange})`,
      timestamp: s.timestamp,
      color: calmChange >= 3 ? "#52B788" : calmChange >= 1 ? "#4CC9F0" : "#7C3AED",
      badges: [
        { text: `${s.duration} min`, color: "#7C3AED" },
        { text: s.technique.replace('_', ' '), color: "#4CC9F0" },
        ...(calmChange >= 3 ? [{ text: `+${calmChange} calm`, color: "#52B788" }] : [])
      ],
      data: s
    };
  });
  ...
  return (
    <PillarPage pillar={PILLAR} title="Meditation Practice" subtitle="Build mindfulness and inner peace" stats={stats}>
      <PillarAtAGlance metrics={atAGlanceMetrics} color={PILLAR.color} />

      <MilestonesSection milestones={milestones} color={PILLAR.color} title="Meditation Achievements" compact={true} />
      ...
      {calmTrendData.some(d => d.improvement !== null) && (
        ...
        >
          ...
          <div className="text-center text-white/60 text-sm mt-2">
            {calmTrendData.filter(d => d.sessions > 0).length} of 14 days practiced
          </div>
        </div>
      )}
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <ActionCard
          ...
          stats={`${sessions.filter(s => s.durationType === 'guided').length} completed`}
          color={PILLAR.color}
          onClick={() => setShowPlayer(true)}
        />
        <ActionCard
      ...
      {showLogger && (
        <MeditationLogger onClose={() => setShowLogger(false)} onSave={(data) => saveSessionMutation.mutateAsync(data)} />
      )}

      {showPlayer && (
        <MeditationPlayer onClose={() => setShowPlayer(false)} onComplete={(data) => saveSessionMutation.mutateAsync(data)} />
      )}

      {selectedLog && (
        <LogDetailModal log={selectedLog} onClose={() => setSelectedLog(null)} color={PILLAR.color} icon={Wind} title="Meditation Session" fields={[
          { key: 'duration', label: 'Duration', icon: Clock, color: "#7C3AED", unit: ' min' },
          { key: 'technique', label: 'Technique', icon: Sparkles, color: "#4CC9F0", render: (value) => <span className="text-lg capitalize text-white/90">{value.replace('_', ' ')}</span> },
          { key: 'calmBefore', label: 'Calm Before', icon: Brain, color: "#FFD700", unit: '/10' },
          { key: 'calmAfter', label: 'Calm After', icon: Heart, color: "#52B788", unit: '/10' },
          { key: 'focusQuality', label: 'Focus Quality', icon: Target, color: "#4CC9F0", unit: '/10' },
          { key: 'mood', label: 'Mood After', icon: Heart, color: "#FF69B4", render: (value) => <span className="text-lg capitalize text-white/90">{value}</span> },
          { key: 'distractions', label: 'Distractions', icon: AlertCircle, color: "#FF5733" },
          { key: 'insights', label: 'Insights', icon: Sparkles, color: "#FFD700" }
        ]} />
      )}
    </PillarPage>
  );
