...
import { createPageUrl, PILLARS, getPillarsArray } from '@/utils';
import { Target, ChevronRight, Calendar, Sparkles, Plus, X, Loader2, TrendingUp, CheckCircle2, ArrowLeft, Search } from "lucide-react";
...
function CreatePlanModal({ pillar, onClose, onSuccess, user }) {
  ...
  const filteredGoals = availableGoals.filter(goal => 
    goal.goalStatement?.toLowerCase().includes(goalSearchQuery.toLowerCase()) ||
    goal.specific?.toLowerCase().includes(goalSearchQuery.toLowerCase())
  );
...
Make actions small, specific, and achievable. Focus on habit formation. Be encouraging and realistic.`;
      ...
      setGeneratedPlan(response.output || response);
      setStep(3);
    } catch (error) {
        ...
        >
          ...
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-2xl font-bold text-white">Create {pillar.name} Plan</h2>
          ...
          {step === 1 && (
            <div className="space-y-6">
              <p className="text-white/80 mb-6">
                Answer a few questions to help us create the perfect plan for you.
              </p>
          ...
          {step === 2 && (
            <div className="space-y-6">
              <div>
                <h3 className="text-lg font-bold text-white mb-2">Link Existing Goals (Optional)</h3>
                <p className="text-white/60 text-sm mb-4">
                  Select SmartGoals to link to this plan. Progress will be tracked automatically.
                </p>
                ...
                {availableGoals.length > 0 ? (
                        ...
                        >
                          <div className="flex items-start gap-3">
                            ...
                            <div className="flex-1">
                              <p className="text-white font-semibold text-sm italic leading-tight">"{goal.goalStatement}"</p>
                              <div className="flex items-center gap-2 mt-1">
                                ...
                                <span className="text-xs text-white/60">{goal.progress || 0}%</span>
                              </div>
                            </div>
                          </div>
                        </button>
                  ...
                  <div className="text-center py-8 bg-white/5 rounded-xl">
                    ...
                    <p className="text-white/60 text-sm">No existing goals for this pillar yet.</p>
                    <p className="text-white/40 text-xs mt-1">You can create goals after your plan is ready.</p>
                  </div>
                )}
              </div>
          ...
          {step === 3 && generatedPlan && (
            <div className="space-y-6">
              <div>
                <h3 className="text-2xl font-bold text-white mb-2">{generatedPlan.planTitle}</h3>
                <p className="text-white/80 leading-relaxed">{generatedPlan.planDescription}</p>
              </div>
              ...
              {selectedGoalIds.length > 0 && (
                <div className="bg-[#D4AF37]/10 border border-[#D4AF37]/30 rounded-xl p-4">
                  ...
                  <p className="text-white/70 text-sm">
                    {selectedGoalIds.length} SmartGoal{selectedGoalIds.length !== 1 ? 's' : ''} will be linked to this plan
                  </p>
                </div>
              )}
              ...
              <div className="space-y-4">
                ...
                {generatedPlan.weeklyActions && generatedPlan.weeklyActions.length > 0 && (
                  <div>
                    <div className="flex items-center gap-2 mb-3">
                ...
                {generatedPlan.successMetrics && generatedPlan.successMetrics.length > 0 && (
                  <div>
                    <div className="flex items-center gap-2 mb-3">
...
export default function MyPlans() {
  ...
  const handleSuccess = (createdPlan) => {
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex items-center gap-4 mb-6">
          ...
          <div className="flex-1">
            ...
            <p className="text-white/60 text-sm">AI-powered transformation roadmaps</p>
          </div>
        </div>
        ...
        {showSuccess && (
          <div className="mb-6 bg-green-500/20 border border-green-500/40 rounded-2xl p-4 text-center animate-in slide-in-from-top duration-300">
            <p className="text-green-400 font-bold">✓ Plan created successfully!</p>
          </div>
        )}
        ...
        {plans.length === 0 ? (
          <div className="text-center py-16">
            <div className="bg-white/5 border border-white/10 rounded-2xl p-12 max-w-2xl mx-auto">
              ...
              <h2 className="text-2xl font-bold text-white mb-2">Create Your First Plan</h2>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {pillarsArray.map(pillar => (
                  ...
                  >
                    ...
                    <div className="text-white font-bold text-sm group-hover:text-[#D4AF37] transition-colors">{pillar.name}</div>
                  </button>
                ))}
              </div>
            </div>
          ...
          <>
            {activePlans.length > 0 && (
              <div className="mb-8">
                <h2 className="text-xl font-bold text-white mb-4">Active Plans ({activePlans.length})</h2>
                <div className="space-y-3">
                  {activePlans.map(plan => {
                    const pillar = pillarsArray.find(p => p.id === plan.pillar);
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-start gap-4">
                          ...
                          <div className="flex-1">
                            <h3 className="text-lg font-bold text-white mb-1 group-hover:text-[#D4AF37] transition-colors">{plan.planTitle}</h3>
                            <p className="text-white/70 text-sm mb-3 line-clamp-2">{plan.planDescription}</p>
                            
                            <div className="flex items-center gap-3 text-xs text-white/60">
                              {plan.startDate && (
                                <>
                                  <Calendar className="w-3 h-3" />
                                  <span>Started {format(new Date(plan.startDate), 'MMM d')}</span>
                                </>
                              )}
                              {plan.linkedGoalsCount > 0 && (
                                <>
                                  <span>•</span>
                                  <span className="text-[#D4AF37]">{plan.linkedGoalsCount} linked goal{plan.linkedGoalsCount !== 1 ? 's' : ''}</span>
                                </>
                              )}
                              {plan.completionProgress > 0 && (
                                <>
                                  <span>•</span>
                                  <span className="text-green-400">{plan.completionProgress}% complete</span>
                                </>
                              )}
                            </div>
                          </div>
            ...
            {completedPlans.length > 0 && (
              <div>
                <h2 className="text-xl font-bold text-white mb-4">Completed Plans ({completedPlans.length})</h2>
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex-1">
                            <h3 className="text-white font-bold">{plan.planTitle}</h3>
