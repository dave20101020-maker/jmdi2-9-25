...
import { CheckCircle2, Circle, Flame, Calendar, TrendingUp, Pause, Play, Edit2, Trash2, Copy, Archive, ChevronDown, ChevronUp, Save, X, AlertCircle, Link, Check, Shield, Bell, BellOff, Clock } from "lucide-react";
import { format, differenceInDays } from "date-fns";
import { toast } from "sonner";
import { PILLARS } from "@/components/shared/Utils";

export default function HabitCard({ habit, onToggleComplete, onEdit, onDelete, onTogglePause, compact = false }) {
  ...
  const [editDifficulty, setEditDifficulty] = useState(habit.difficulty || 'medium');
  const [editReminderEnabled, setEditReminderEnabled] = useState(habit.reminderEnabled ?? true);
  ...
  const daysSinceLastCompletion = lastCompletedDate ? differenceInDays(new Date(), lastCompletedDate) : 999;
  const isStreakAtRisk = daysSinceLastCompletion > 1 && habit.streakCount > 0;
  
  const completionRate = habit.totalCompletions > 0 && habit.created_date
    ? Math.round((habit.totalCompletions / Math.max(1, differenceInDays(new Date(), new Date(habit.created_date)))) * 100)
    : 0;
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              <h4 className="text-white font-semibold truncate flex-1">{habit.habitText}</h4>
              {hasLinks && (
            ...
            <div className="flex items-center gap-2 text-xs text-white/60">
              <span className="flex items-center gap-1">
                ...
                {forgivenessUsed && (
                  <Shield className="w-3 h-3 text-blue-400" title="Forgiveness used this week" />
                )}
              </span>
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-start gap-3 flex-1">
              ...
              <AnimatePresence mode="wait">
                {isCompletedToday ? (
                  ...
                  <motion.div
                    ...
                    exit={{ scale: 0 }}
                  >
                    <Circle className="w-7 h-7 text-white/40" aria-hidden="true" />
                  </motion.div>
                )}
              </AnimatePresence>
            </motion.button>
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-1">
                <h3 className="text-lg font-bold text-white flex-1">{habit.habitText}</h3>
                {hasLinks && (
                  <div className="flex items-center gap-1 px-2 py-1 bg-[#D4AF37]/20 border border-[#D4AF37]/40 rounded-full">
                    <Link className="w-3 h-3 text-[#D4AF37]" />
                    <span className="text-xs text-[#D4AF37] font-bold">Linked</span>
                  </div>
                )}
              </div>
        ...
        >
          {[
            { icon: Flame, label: "Streak", value: habit.streakCount, color: "orange", sub: forgivenessUsed ? 'Protected' : `Best: ${habit.bestStreak || habit.streakCount}`, hasShield: forgivenessUsed },
            { icon: CheckCircle2, label: "Total", value: habit.totalCompletions || 0, color: "green", sub: "completions" },
            { icon: TrendingUp, label: "Rate", value: `${completionRate}%`, color: "blue", sub: "success" }
          ].map((stat, idx) => (
            <motion.div 
              key={idx}
          ...
          >
            <div className="p-5 space-y-4">
                ...
                <>
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      <Label className="text-white font-bold">Habit Description</Label>
                      <span className={`text-xs ${editText.length > 200 ? 'text-red-400' : 'text-white/60'}`}>
                        {editText.length}/200
                      </span>
                    </div>
                    <Input
                  ...
                  <div>
                    <Label className="text-white mb-2 block font-bold">Frequency</Label>
                  ...
                  <div>
                    <Label className="text-white mb-2 block font-bold">Difficulty</Label>
                  ...
                  <div className="bg-blue-500/10 border border-blue-500/30 rounded-xl p-4 space-y-3">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <Label className="text-white font-bold">Daily Reminder</Label>
                      </div>
                      <button
                        ...
                        aria-label={editReminderEnabled ? "Disable reminder" : "Enable reminder"}
                      >
                        <motion.span
                      ...
                      {editReminderEnabled && (
                        ...
                        >
                          ...
                          <p className="text-xs text-white/60 mt-2">
                            You'll get a notification at {editReminderTime} to complete this habit
                          </p>
                        </motion.div>
                      )}
                    </AnimatePresence>
                ...
                <>
                  ...
                  <div className="space-y-3 text-sm">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-white font-medium capitalize">{habit.frequency}</span>
                    </div>
                    
                    <div className="flex items-center justify-between">
                      <span className="text-white/70">Difficulty:</span>
                      <span className={`font-medium capitalize ${
                        habit.difficulty === 'easy' ? 'text-green-400' :
                        habit.difficulty === 'hard' ? 'text-red-400' :
                        'text-yellow-400'
                      }`}>{habit.difficulty || 'medium'}</span>
                    </div>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span className={`font-medium ${habit.reminderEnabled ? 'text-blue-400' : 'text-white/60'}`}>
                    ...
                    {forgivenessUsed && (
                      ...
                      >
                        <div className="flex items-center gap-2 text-blue-400 text-sm font-semibold">
                          ...
                          1 forgiveness used this week - streak protected!
                        </div>
                      </motion.div>
                    )}
              ...
              <div className="flex items-center gap-3 text-xs text-white/40 pt-3 border-t border-white/10">
                ...
                <span>Created {format(new Date(habit.created_date), 'MMM d, yyyy')}</span>
              </div>
            </div>
          </motion.div>
        )}
