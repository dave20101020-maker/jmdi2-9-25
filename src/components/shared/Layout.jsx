...
import { Home, TrendingUp, MessageCircle, User, Plus, Target, Users, Trophy, Flame, Bell } from "lucide-react";
      ...
      <div className="h-full overflow-y-auto pb-20 pt-6 px-4 sm:px-6 safe-bottom">
        <div className="max-w-md mx-auto">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-xl sm:text-2xl font-bold text-white">Track a Pillar</h2>
            <button 
              onClick={onClose} 
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1 text-left">
                    <h3 className="text-lg font-semibold text-white transition-colors group-hover:text-white">{pillar.name}</h3>
                    <p className="text-sm text-white/60 group-hover:text-white/80 transition-colors">Quick log &rarr;</p>
                  </div>
                </div>
              </button>
            ))}
...
function BottomNav() {
  ...
  useEffect(() => {
    ...
    const hasSeenPromptToday = sessionStorage.getItem('reflectionPromptShown') === format(today, 'yyyy-MM-dd');
    
    if (isSunday && !hasSeenPromptToday && location.pathname === createPageUrl("Dashboard")) {
      setTimeout(() => {
        ...
        sessionStorage.setItem('reflectionPromptShown', format(today, 'yyyy-MM-dd'));
      }, 2000);
    }
  }, [location.pathname, navigate]);
  
  const navItems = [
    { icon: Home, label: "Home", path: createPageUrl("Dashboard"), ariaLabel: "Go to home dashboard" },
    { icon: Trophy, label: "Growth", path: createPageUrl("MyGrowth"), ariaLabel: "View growth goals and plans" },
    { icon: Plus, label: "Track", path: null, action: () => setShowPillarSelector(true), ariaLabel: "Track a pillar" },
    { icon: TrendingUp, label: "Insights", path: createPageUrl("Analytics"), ariaLabel: "View analytics and insights" },
    { icon: User, label: "Profile", path: createPageUrl("Profile"), ariaLabel: "View your profile" }
  ];
            ...
            {navItems.map((item, idx) => {
              ...
              if (item.action) {
                return (
                  ...
                  >
                    <div className={`${isMiddle ? 'w-14 h-14 -mt-6 rounded-full flex items-center justify-center shadow-lg transition-all duration-300 group-hover:scale-110 group-active:scale-95' : ''}`}
                      style={isMiddle ? { 
                        background: `linear-gradient(to bottom right, ${COLORS.PRIMARY}, ${COLORS.PRIMARY_LIGHT})`,
                        boxShadow: `0 0 20px ${COLORS.PRIMARY}80`
                      } : {}}
                    >
                      <Icon className={`${isMiddle ? 'w-7 h-7' : 'w-6 h-6 text-white/60 group-hover:text-white'} transition-colors`} style={isMiddle ? { color: COLORS.BACKGROUND } : {}} aria-hidden="true" />
                    </div>
                    {!isMiddle && <span className="text-xs text-white/60 group-hover:text-white transition-colors">{item.label}</span>}
                  </button>
                );
              }
              ...
              return (
                ...
                >
                  <Icon className={`w-6 h-6 transition-all duration-200`} style={{ color: isActive ? COLORS.PRIMARY : 'rgba(255,255,255,0.6)' }} aria-hidden="true" />
                  <span className={`text-xs transition-all duration-200`} style={{ color: isActive ? COLORS.PRIMARY : 'rgba(255,255,255,0.6)' }}>
                    {item.label}
                  </span>
                  {isActive && (
                    <div className="absolute -top-1 w-1 h-1 rounded-full animate-pulse" 
                      style={{ backgroundColor: COLORS.PRIMARY, boxShadow: `0 0 8px ${COLORS.PRIMARY}CC` }}
                      aria-hidden="true"
                    />
                  )}
                </Link>
...
export default function Layout({ children }) {
  ...
  useEffect(() => {
    async function checkAuth() {
      try {
        ...
        if (!currentUser.onboarding_completed && !location.pathname.includes('Onboarding')) {
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ background: `linear-gradient(to bottom, ${COLORS.BACKGROUND}, #1A1838)` }}>
        <div className="text-white text-center">
          <div className="w-16 h-16 mx-auto mb-4 relative">
            ...
            <div className="relative w-16 h-16 rounded-full flex items-center justify-center text-3xl pulse-glow"
              style={{ background: `linear-gradient(to bottom right, ${COLORS.PRIMARY}, ${COLORS.PRIMARY_LIGHT})` }}
            >
              ⭐
            </div>
          </div>
  ...
  return (
    <div className="min-h-screen relative overflow-hidden safe-top safe-bottom safe-left safe-right" style={{ background: `linear-gradient(to bottom, ${COLORS.BACKGROUND}, #1A1838)` }}>
      ...
      {!isOnboarding && user && (
        <header className="fixed top-0 left-0 right-0 z-30 backdrop-blur-lg border-b border-white/10 safe-top" style={{ backgroundColor: `${COLORS.BACKGROUND}CC` }} role="banner">
          <div className="max-w-md mx-auto px-4 sm:px-6 py-2 flex items-center justify-between">
            <div className="flex items-center gap-3 sm:gap-4" role="status" aria-label="User stats">
              <div className="flex items-center gap-1 group">
                <Trophy className="w-4 h-4 transition-all duration-300 group-hover:scale-110 group-hover:rotate-3" style={{ color: COLORS.PRIMARY }} aria-hidden="true" />
                <span className="text-sm font-bold transition-colors" style={{ color: COLORS.PRIMARY_LIGHT }}>{user.points || 0}</span>
              ...
              {user.streak_days > 0 && (
                <>
                  ...
                  <div className="flex items-center gap-1 group">
                    <Flame className="w-4 h-4 text-orange-400 transition-all duration-300 group-hover:scale-110" aria-hidden="true" />
                    <span className="text-sm font-bold text-orange-400">{user.streak_days}</span>
                    <span className="text-xs text-white/60">day{user.streak_days !== 1 ? 's' : ''}</span>
                  </div>
                </>
              )}
            </div>
            ...
            <div>
              <Link to="/notifications" className="relative inline-flex items-center p-2 rounded hover:bg-white/3">
                <Bell className="w-5 h-5 text-white/80" />
                {notificationsCount > 0 && (
                  <span className="absolute -top-1 -right-1 inline-flex items-center justify-center px-1.5 py-0.5 text-xs rounded-full bg-red-500 text-white">{notificationsCount}</span>
                )}
              </Link>
            </div>
          </div>
      ...
      <main className="relative z-10 min-h-screen pb-20" style={!isOnboarding && user ? { paddingTop: '3rem' } : {}}>
        {children}
      </main>
