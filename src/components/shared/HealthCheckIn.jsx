...
import { X, Save, Heart, Activity, Utensils, CheckCircle2 } from "lucide-react";
...
const CONCERN_LEVELS = [
  ...
  { value: "needs_attention", label: "Needs Attention", color: "#FF5733", emoji: "⚠️" }
];
...
export default function HealthCheckIn({ onClose, onSave }) {
  ...
  const addSymptom = () => {
    if (symptomInput.trim()) {
      setCheckIn({
        ...checkIn,
        currentSymptoms: [...(checkIn.currentSymptoms || []), symptomInput.trim()]
      });
      setSymptomInput("");
    }
  };
  ...
  const painColor = checkIn.painLevel === 0 ? '#52B788' :
                    checkIn.painLevel <= 3 ? '#FFD700' : '#FF5733';
        ...
        <div className="p-6 space-y-6">
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="font-bold text-lg" style={{ color: energyColor }}>
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-3">
              {DIGESTION_OPTIONS.map(option => (
                <button
                  key={option.value}
                  onClick={() => setCheckIn({ ...checkIn, digestion: option.value })}
            ...
            {checkIn.currentSymptoms?.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {checkIn.currentSymptoms.map((symptom, idx) => (
                  ...
                  >
                    {symptom}
                    <button onClick={() => removeSymptom(idx)} className="hover:text-red-400">
                      <X className="w-3 h-3" />
                    </button>
                  </span>
                ))}
          ...
          <div className="flex items-center justify-between bg-white/5 border border-white/10 rounded-xl p-4">
            <div>
              ...
              <div className="text-white/60 text-sm">Did you do any physical activity?</div>
            </div>
            <button
              className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                checkIn.exerciseCompleted ? 'bg-green-500' : 'bg-white/20'
              }`}
              onClick={() => setCheckIn({ ...checkIn, exerciseCompleted: !checkIn.exerciseCompleted })}
            >
              <span
          ...
          {/* Concern Level */}
          <div>
            <Label className="text-white mb-3 block">Overall Concern Level</Label>
            <div className="grid grid-cols-2 gap-3">
              {CONCERN_LEVELS.map(concern => (
                <button
                  key={concern.value}
                  onClick={() => setCheckIn({ ...checkIn, concernLevel: concern.value })}
                  className={`p-4 rounded-xl border-2 transition-all ${
                    checkIn.concernLevel === concern.value
                      ? 'border-2 scale-105'
                      : 'bg-white/5 border-white/10 hover:bg-white/10'
                  }`}
                  style={checkIn.concernLevel === concern.value ? {
                    backgroundColor: `${concern.color}20`,
                    borderColor: concern.color
                  } : {}}
                >
                  <div className="text-2xl mb-1">{concern.emoji}</div>
                  <div className="text-white text-sm font-bold">{concern.label}</div>
                </button>
              ))}
            </div>
          </div>
