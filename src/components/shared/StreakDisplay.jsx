...
export default function StreakDisplay({ streak, user, compact = false }) {
  ...
  const handleFreeze = async () => {
    if (!streak.canFreeze) {
      toast.error("No momentum pauses available");
      return;
    }

    const confirmed = window.confirm(
      `Use a momentum pause? You have ${streak.freezesRemaining} pause${
        streak.freezesRemaining !== 1 ? "s" : ""
      } remaining this month.\n\nThis will protect your momentum if you miss tomorrow.`
    );
    ...
    try {
      ...
      toast.success(
        "Momentum pause activated — you're protected for tomorrow."
      );
    } catch (error) {
      console.error("Failed to freeze streak:", error);
      toast.error("Couldn't pause momentum. Please try again.");
    } finally {
      setFreezing(false);
    }
  };
      ...
      <div className="relative">
        ...
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-white/60 text-sm">Current Momentum</div>
        ...
        <div className="mb-4">
          {streak.todayLogged ? (
            <div className="flex items-center gap-2 text-green-400 text-sm font-medium">
              ...
              Logged today — momentum active
            </div>
          ) : (
            <div className="flex items-center gap-2 text-orange-400 text-sm font-medium">
              <div className="w-2 h-2 rounded-full bg-orange-400" />
              Momentum resumes with your next entry
            </div>
          )}
        </div>
          ...
          <div className="mt-4 p-3 bg-blue-500/10 border border-blue-500/30 rounded-lg flex items-start gap-2">
            ...
            <div className="text-xs text-blue-400">
              <div className="font-semibold mb-1">
                No momentum pauses available
              </div>
              <div className="text-blue-400/80">
                Pauses reset monthly. Premium users get 3/month, Free users get
                1/month.
              </div>
            </div>
          </div>
