...
import { Target, CheckCircle2, Repeat, ChevronRight, TrendingUp } from "lucide-react";
import { Link } from "react-router-dom";
import { createPageUrl, PILLARS } from "@/components/shared/Utils";

export default function ActiveItemsWidget({ plans = [], goals = [], habits = [] }) {
  const activePlans = plans.filter(p => p.isActive);
  const activeGoals = goals.filter(g => g.status === 'active');
  const activeHabits = habits.filter(h => h.isActive);

  const totalActive = activePlans.length + activeGoals.length + activeHabits.length;
  ...
  if (totalActive === 0) {
    return (
      ...
      >
        ...
        <p className="text-white/60 text-sm mb-4">No active plans, goals, or habits yet</p>
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-4">
        ...
        <Link to={createPageUrl("MyGrowth")} className="text-[#D4AF37] text-sm hover:underline">
          View All
        </Link>
      </div>
      ...
      <div className="space-y-3">
        ...
        {activePlans.slice(0, 2).map((plan) => {
          ...
          return (
            <Link key={plan.id} to={createPageUrl("PlanDetail") + `?id=${plan.id}`}>
              <motion.div
              ...
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <h4 className="text-white font-semibold text-sm mb-2 line-clamp-1">{plan.planTitle}</h4>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-white/60 whitespace-nowrap">{plan.completionProgress || 0}%</span>
                    </div>
                  </div>
        ...
        {activeGoals.slice(0, 2).map((goal) => {
          ...
          return (
            <Link key={goal.id} to={createPageUrl("Goals") + `?goalId=${goal.id}`}>
              <motion.div
                className="bg-white/5 border border-white/10 rounded-xl p-4 hover:bg-white/10 transition-all group cursor-pointer"
                whileHover={{ x: 3 }}
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <h4 className="text-white font-semibold text-sm mb-2 line-clamp-2">{goal.specific || goal.goalStatement}</h4>
                    <div className="flex items-center gap-2">
                      <div className="flex-1 h-1.5 bg-white/10 rounded-full overflow-hidden">
                        <div
                          className="h-full rounded-full transition-all"
                      ...
                      <span className="text-xs text-white/60 whitespace-nowrap">{goal.progress || 0}%</span>
                    </div>
                  </div>
                  <ChevronRight className="w-5 h-5 text-white/40 group-hover:text-white/80 transition-colors flex-shrink-0" />
                </div>
              ...
              >
                <div className="flex items-start gap-3">
                  <div
                    ...
                    style={{ backgroundColor: `${pillar?.color}20` }}
                  >
                    <Repeat className="w-5 h-5" style={{ color: pillar?.color }} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-xs px-2 py-0.5 rounded-full" style={{ backgroundColor: `${pillar?.color}20`, color: pillar?.color }}>
                    ...
                    <h4 className="text-white font-semibold text-sm mb-2 line-clamp-2">{habit.habitText}</h4>
                    <div className="flex items-center gap-3">
                      <div className="flex items-center gap-1">
                        <span className="text-orange-400 text-lg">🔥</span>
                        <span className="text-sm font-bold text-orange-400">{habit.streakCount || 0}</span>
