...
export const getPillarsArray = () => {
  return Object.keys(PILLARS).map((id) => ({
    id,
    ...PILLARS[id],
  }));
};
...
export const calculateAllScores = (entries) => {
  return {};
};

export const createPageUrl = (pageName) => getRoutePath(pageName);
...
export const calculateHabitStats = (habits) => {
  ...
  const completionRates = activeHabits.map((h) => {
    ...
    const daysSinceCreation = Math.max(
      1,
      differenceInDays(new Date(), new Date(h.created_date))
    );
